...
(function ($) {
  ...
  function localizeLoadingText() {
    ...
    if ($.magnificPopup && $.magnificPopup.defaults) {
      ...
      $.magnificPopup.defaults.tClose = "Закрити (Esc)";
    }
  ...
  localizeLoadingText();
  $(document).on(
    "mfpOpen mfpBeforeOpen mfpUpdateStatus ajaxComplete",
    function () {
      localizeLoadingText();
    },
  );
  ...
  function promotional_bar() {
    var writePromotionCookie = function () {
      if (
  ...
  function minicart_dropdown() {
    $(".site-header__cart").on("click", function (i) {
      ...
      var orderUrl = String($(this).attr("data-order-url") || "");
      var hasHeaderCart = $("#header-cart").length > 0;

      // If cart link points to an order page, keep native navigation.
      if (orderUrl !== "" || (href !== "" && href !== "#")) {
        return;
      }
  ...
  function product_thumb1() {
    var $thumbSlider = $(".product-dec-slider-1");

    if (!$thumbSlider.length || !$.fn.slick) {
      return;
    }
  ...
  /*--------------------------
      25. Product Page Popup ---
	---------------------------- */
  function product_lightbox() {
    $(".prlightbox")
      .off("click.deLightbox")
      .on("click.deLightbox", function (e) {
        e.preventDefault();

        var $productWrap = $(this).closest(".product-details-img");
        var $anchors = $productWrap.find(".lightboximages a[href]");
        var items = [];
        var usePhotoSwipe =
        ...
        $anchors.each(function () {
          ...
          var size = String($(this).attr("data-size") || "").split("x");
          ...
          if (src) {
            items.push({ src: src, w: width, h: height });
          }
        });

        if (!items.length) {
          var fallbackSrc = $productWrap.find(".zoompro").attr("src");

          if (fallbackSrc) {
            items.push({ src: fallbackSrc, w: 1200, h: 1200 });
          }
        }
        ...
        if (usePhotoSwipe) {
          var $pswp = $(".pswp").first();
          var currentIndex = 0;
          var currentSrc = $productWrap.find(".zoompro").attr("src") || "";

          if (
            $productWrap.find("#gallery .slick-current [data-image]").length
          ) {
            currentSrc =
              $productWrap
                .find("#gallery .slick-current [data-image]")
                .attr("data-image") || currentSrc;
          }

          $.each(items, function (index, item) {
            if (String(item.src) === String(currentSrc)) {
              currentIndex = index;
              return false;
            }
        ...
        if (!$.fn.magnificPopup || !$.magnificPopup) {
          return;
        }
  ...
  function cookie_promo() {
    ...
    var cookieWriter = function (key, value) {
      if (
        typeof Cookies !== "undefined" &&
        Cookies &&
        typeof Cookies.set === "function"
    ...
    if (cookieReader("promotion") != "true") {
      $(".notification-bar").show();
    }

    $(".close-announcement").on("click", function () {
      $(".notification-bar").slideUp();
      cookieWriter("promotion", "true");
      return false;
    });
  }
  cookie_promo();
